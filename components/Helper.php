...
class Helper extends Component
{
    ...
    {
        $content = str_replace('data-blogger-escaped-target', 'target', $text);
        $mh_url_regex = "/\<a\ href\=\"(http|https)\:\/\/[a-zA-Z0-9\-\.]+[a-zA-Z]{2,3}.*\"[\ >]/";
        preg_match_all($mh_url_regex, $text, $mh_matches);
        ...
        for ($mh_count = 0; $mh_count < count($mh_matches[0]); $mh_count++) {
            ...
            if (!$this->mh_array_find($mh_old_url, $mh_ignore))
                $content = str_replace($mh_old_url, $mh_new_url, $content);
        }
        return $content;
    }

    public function mh_array_find($needle, $haystack)
    {
