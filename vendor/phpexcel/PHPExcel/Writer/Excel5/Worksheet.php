...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
			...
			if ($cVal instanceof PHPExcel_RichText) {
				...
				$arrcRun = array();
				$str_len = String::CountCharacters($cVal->getPlainText(), 'UTF-8');
				$str_pos = 0;
				...
				foreach ($elements as $element) {
					...
					// Position FROM
					$str_pos += String::CountCharacters($element->getText(), 'UTF-8');
				}
	...
	private function _writeRichTextString($row, $col, $str, $xfIndex, $arrcRun){
		...
		$str = String::UTF8toBIFF8UnicodeShort($str, $arrcRun);
	...
	private function _writeLabelSst($row, $col, $str, $xfIndex)
	{
		...
		$str = String::UTF8toBIFF8UnicodeLong($str);
	...
	private function _writeStringRecord($stringValue)
	{
		...
		$data = String::UTF8toBIFF8UnicodeLong($stringValue);
	...
	function _writeUrlInternal($row1, $col1, $row2, $col2, $url)
	{
		...
		// character count
		$url_len = String::CountCharacters($url);
		$url_len = pack('V', $url_len);

		$url = String::ConvertEncoding($url, 'UTF-16LE', 'UTF-8');
	...
	private function _writeRangeProtection()
	{
		foreach ($this->_phpSheet->getProtectedCells() as $range => $password) {
			...
			$recordData .= String::UTF8toBIFF8UnicodeLong('p' . md5($recordData));
	...
	private function _writeHeader()
	{
		...
		*/
		...
		$recordData = String::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddHeader());
		$length = strlen($recordData);
	...
	private function _writeFooter()
	{
		...
		*/
		...
		$recordData = String::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddFooter());
		$length = strlen($recordData);
	...
	private function _writeDataValidity()
	{
		...
		if (!empty($dataValidationCollection)) {
			...
			foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
				...
				$data .= String::UTF8toBIFF8UnicodeLong($promptTitle);

				// error title
				$errorTitle = $dataValidation->getErrorTitle() !== '' ?
					$dataValidation->getErrorTitle() : chr(0);
				$data .= String::UTF8toBIFF8UnicodeLong($errorTitle);

				// prompt text
				$prompt = $dataValidation->getPrompt() !== '' ?
					$dataValidation->getPrompt() : chr(0);
				$data .= String::UTF8toBIFF8UnicodeLong($prompt);

				// error text
				$error = $dataValidation->getError() !== '' ?
					$dataValidation->getError() : chr(0);
				$data .= String::UTF8toBIFF8UnicodeLong($error);
	...
	private function _writeCFRule(PHPExcel_Style_Conditional $conditional){
		...
		if($bFormatFont == 1){
			...
			if($conditional->getStyle()->getFont()->getName() == null){
				...
				$dataBlockFont = String::UTF8toBIFF8UnicodeLong($conditional->getStyle()->getFont()->getName());
			}
			// Font Size
