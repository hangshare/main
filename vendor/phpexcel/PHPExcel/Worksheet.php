...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
   ...
   /**
    ...
    private static function _checkSheetTitle($pValue)
    {
        ...
        if (String::CountCharacters($pValue) > 31) {
    ...
    public function setTitle($pValue = 'Worksheet', $updateFormulaCellReferences = true)
    {
        ...
        if ($this->_parent) {
			...
			if ($this->_parent->sheetNameExists($pValue)) {
                ...
                if (String::CountCharacters($pValue) > 29) {
                    $pValue = String::Substring($pValue,0,29);
                }
                $i = 1;
				while ($this->_parent->sheetNameExists($pValue . ' ' . $i)) {
                    ++$i;
                    if ($i == 10) {
                        if (String::CountCharacters($pValue) > 28) {
                            $pValue = String::Substring($pValue,0,28);
                        }
                    } elseif ($i == 100) {
                        if (String::CountCharacters($pValue) > 27) {
                            $pValue = String::Substring($pValue,0,27);
                        }
                    }
                }
