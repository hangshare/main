				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					if (isset($xmlStrings) && isset($xmlStrings->si)) {
						foreach ($xmlStrings->si as $val) {
							if (isset($val->t)) {
								$sharedStrings[] = String::ControlCharacterOOXML2PHP( (string) $val->t );
							} elseif (isset($val->r)) {
...
//		}
	...
	private function _parseRichText($is = null) {
		...
		if (isset($is->t)) {
			$value->createText( String::ControlCharacterOOXML2PHP( (string) $is->t ) );
		} else {
			foreach ($is->r as $run) {
				if (!isset($run->rPr)) {
					$objText = $value->createText( String::ControlCharacterOOXML2PHP( (string) $run->t ) );

				} else {
					$objText = $value->createTextRun( String::ControlCharacterOOXML2PHP( (string) $run->t ) );
