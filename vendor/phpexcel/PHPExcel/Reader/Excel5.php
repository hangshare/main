		...
		for ($i = 0; $i < $countProperties; ++$i) {
			...
			switch ($type) {
				...
				case 0x1E: // null-terminated string prepended by dword string length
					...
					$value = String::ConvertEncoding($value, 'UTF-8', $codePage);
					$value = rtrim($value);
					break;
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
				...
				for ($i = 0; $i <= $sstCount; ++$i) {
					if (isset($fmtRuns[$i])) {
						$text = String::Substring($this->_sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
						$charPos = $fmtRuns[$i]['charPos'];
					} else {
						$text = String::Substring($this->_sst[$index]['value'], $charPos, String::CountCharacters($this->_sst[$index]['value']));
					}

					if (String::CountCharacters($text) > 0) {
...
//			var_dump($bcBitCount);
	...
	private static function _encodeUTF16($string, $compressed = '')
	{
		...
		return String::ConvertEncoding($string, 'UTF-8', 'UTF-16LE');
	}
	...
	{
		return String::ConvertEncoding($string, 'UTF-8', $this->_codepage);
	}
